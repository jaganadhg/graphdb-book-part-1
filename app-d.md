# Appendix D: Generating more synthetic data with SDV

Appendix A's dataset is deliberately tiny: small enough to whiteboard. For three other purposes you will want a *larger* graph: rehearsing traversal at realistic depth, demonstrating query performance, and stress-testing a load pipeline. This appendix provides a Python script that scales the dataset up using the Synthetic Data Vault (SDV).

## What SDV does here, and what it deliberately does not

SDV is a *tabular* synthesizer: it learns the statistical distribution of a table's columns and samples new rows that resemble it. That is exactly the right tool for one half of this problem and the wrong tool for the other.

- **SDV synthesizes the node populations.** The script fits a `GaussianCopulaSynthesizer` on the seed node tables and samples new rows, so the synthetic `TableAsset` and `Pipeline` populations carry the same property mix (layer, domain, schedule, mode) as the seed, at whatever scale you ask for.
- **Explicit graph logic builds the topology.** Relationships, referential integrity, and medallion ordering (raw → curated → mart) are constructed by code, not by SDV. A tabular synthesizer has no concept of a foreign key that must resolve, or a dependency that must flow in one direction.

This split is not a workaround: it is the same "right tool for the job" judgment the book argues for throughout. It is also why the script does **not** use SDV's relational `HMASynthesizer`: that synthesizer assumes a hierarchical, parent-child schema, and this metadata graph is cyclic and multi-parent. A `TableAsset` is contained by a `Database`, written by a `Pipeline`, and read by other pipelines. SDV's relational model does not fit a general graph.

**A caveat worth stating to the team.** Because the topology is assembled from synthesized populations, the larger graph may contain dependencies a hand-curated platform would not, for instance a curated table reading a raw table of a different domain. That is acceptable for scale and performance testing, which is the point of this appendix. For *teaching the model's semantics*, keep using the Appendix A dataset.

Because the generator is **additive** (it keeps every seed node) all of the named queries in Parts IV and V still run against the expanded graph; they simply traverse a denser one.
...
Place the script below in `scripts/generate_synthetic.py`, with the Appendix A CSVs in `data/`. Run it:

```bash
python scripts/generate_synthetic.py
```

**Using uv (recommended).** For a reproducible, isolated environment, [uv](https://docs.astral.sh/uv/) is a cleaner alternative to a bare `pip install`. From your workspace root:

```bash
uv init                       # creates pyproject.toml, once
uv add sdv pandas numpy       # resolves and locks the dependencies
uv run python scripts/generate_synthetic.py
```

`uv run` executes the script inside the project's environment with no separate activation step, and `uv.lock` makes the environment reproducible for the next person. If you would rather not keep any project files, a single ephemeral run works too:

```bash
uv run --with sdv --with pandas --with numpy python scripts/generate_synthetic.py
```

It writes the expanded dataset (same filenames, same format as Appendix A) into a new `data_synth/` folder, and prints a node/edge count summary and a referential-integrity check. To load it, run the Part III `schema.cypher` against a fresh database, then run a copy of `load.cypher` with the paths pointing at `data_synth/` instead of `data/`. Adjust the scale with the config block at the top of the script.
...
The script was written against SDV 1.x, which uses the unified `Metadata` class. If you are on an older release, consult the SDV documentation for the equivalent metadata API.
